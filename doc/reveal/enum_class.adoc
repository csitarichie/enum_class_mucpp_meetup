:description: Lighting talk for MUC++ meetup
:backend: revealjs
:author: Richard Szabo
:twitter: csitarichie
:blog: https://github.com/csitarichie
...
*scoped enums vs. unscoped enums*

//+++<small>+++
//by {blog}[{author}] / http://twitter.com/{twitter}[@{twitter}]#
//+++</small>+++


== How wrote code using:

[%step]
* C enums
* C++98 enums
* C++11 enum class only
...
[source, c++]
----
  enum Person { joe, pete, jack };  // joe, pete, jack are
                                    // in same scope as Person

  auto pete = true;                 // error! pete already
                                    // declared in this scope

----

Pitfalls of an enum technology of the 80's

[NOTE.speaker]
--
C++98-style enums name belong to the scope containing enum.

--

== implicit type conversation to integral types


[source, c++]
----
 enum Person { joe, pete, jack };  // unscoped enum

 std::vector<std::size_t>          // func. returning
   primeFactors(std::size_t);      // prime factors of x

 Person p = pete;

 if (p < 17.5) {     // compare Person to double (!)

 auto factors =      // compute prime factors
   primeFactors(c)   // of a Person (!)
----
...
[NOTE.speaker]
--
C++98-style enums implicit convert to integral types.
